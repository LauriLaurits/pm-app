 ...
 * open it at all, the admin-only PM reassignment list, and the client/contact pickers. Lives here
 * (rather than inline in layout.tsx) because it used to be fetched by the overview page.tsx only
 * -- now that the dialog mounts in the shared [id] layout so it shows on every tab, both layout.tsx
 * and page.tsx would otherwise need their own copy.
 *
...
export async function getProjectEditData(
  ...
  // can never open the dialog (or the PM field within it) anyway.
  ...
  const [{ data: pmCandidateRows }, { data: clients }, { data: contactOptions }, { data: pmRow }] =
    await Promise.all([
      isAdmin
